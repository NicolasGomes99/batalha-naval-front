...
console.log("   Available (dock):", s1.availableShips.length, "— expected 6");
console.log("   Placed (board):", s1.placedShips.length, "— expected 0");
console.log("   allShipsPlaced:", s1.allShipsPlaced(), "— expected false");
console.log("");

// ── Test 2: Place a ship ─────────────────────────────────────────────────────
const patrulhaId = s1.availableShips.find(
  (s) => s.type === ShipType.SUBMARINO,
)!.id;
console.log("✅ Test 2: Place Submarino (size 1) at (0, 0) Horizontal");
...
console.log("   Available:", s2.availableShips.length, "— expected 5");
console.log("   Placed:", s2.placedShips.length, "— expected 1");
console.log("");

// ── Test 3: Out-of-bounds ────────────────────────────────────────────────────
const portaAvId = s2.availableShips.find(
  (s) => s.type === ShipType.PORTA_AVIAO_A,
)!.id;
console.log(
  "✅ Test 3: Place Porta-Aviões Alpha (size 6) at x=8, y=0 Horizontal — should fail",
);
...
// ── Test 4: Collision ────────────────────────────────────────────────────────
const encId = useSetupStore
  .getState()
  .availableShips.find((s) => s.type === ShipType.ENCOURACADO)!.id;
console.log(
  "✅ Test 4: Place Encouraçado (size 4) at (0, 0) — overlaps Patrulha",
);
...
console.log(
  "   Placed count:",
  useSetupStore.getState().placedShips.length,
  "— expected 2",
);
console.log("");
...
const pShip = useSetupStore
  .getState()
  .placedShips.find((s) => s.id === patrulhaId);
...
console.log("✅ Test 7: Remove Submarino (board → dock)");
useSetupStore.getState().removeShip(patrulhaId);
const s7 = useSetupStore.getState();
console.log("   Available:", s7.availableShips.length, "— expected 5");
...
console.log("   Available:", s8.availableShips.length, "— expected 6");
...
console.log(
  "   (0,0) H size 3, empty board:",
  isValidPlacement({ size: 3 }, 0, 0, ShipOrientation.HORIZONTAL, []),
  "— expected true",
);
console.log(
  "   (8,0) H size 3, empty board:",
  isValidPlacement({ size: 3 }, 8, 0, ShipOrientation.HORIZONTAL, []),
  "— expected false",
);
console.log(
  "   (-1,0) H size 2:",
  isValidPlacement({ size: 2 }, -1, 0, ShipOrientation.HORIZONTAL, []),
  "— expected false",
);
console.log("");
